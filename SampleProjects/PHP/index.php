    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        switch($_POST['ApiType']){
            case 'SendSMS':            
                ...
                $R = $SMS->SendArray($numbers, $_POST['SendArrayMessageBody'], $_POST['SendArraySpecialNumber'], isset($_POST['SendArrayIsFlash']), $checkNumber);
                ...
                foreach($R as $i) {
                    $ID++;
                    $Result_SendSMS = $Result_SendSMS . ($ID + 1) . '. ' . $numbers[$ID] . ' : ' . $i;
                }

                break;
            case 'SendSMSSchedule':
                $numbers= explode(';', $_POST['SendArrayScheduleRecipientNumber']);
                $checkNumber= explode(';', $_POST['SendArrayScheduleCheckingMessageID']);
                ...
                $Result_SendSMSSchedule = $SMS->SendArraySchedule($numbers, $_POST['SendArrayScheduleMessageBody'], $_POST['SendArrayScheduleSpecialNumber'], isset($_POST['SendArrayScheduleIsFlash']), $checkNumber, $Year, $Month, $Day, $Hour, $Minute);
            ...
            case 'GetMessageIDActive':
                $numbers= explode(';', $_POST['SendArrayScheduleRecipientNumber']);
                $R = $SMS->GetMessageID($numbers);
                
                $ID = -1;
                foreach($R as $i) {
                    $ID++;
                    $Result_GetMessageID = $Result_GetMessageID . $ID . '. ' . $numbers[$ID] . ' : ' . $i;
                }
            ...
            case 'GetMessageStatusAction':
                ...
                $R = $SMS->GetMessageStatus($numbers);
                
                $ID = -1;
                foreach($R as $i) {
                    $ID++;
                    $Result_GetMessageStatus = $Result_GetMessageStatus . $ID . '. ' . $numbers[$ID] . ' : ' . $i;
                }

                
                break;
            case 'GetNumberGroupDataAction':
                $Result_GetNumberGroupData = $SMS->GetNumberGroupData();
                $Status = 'ارسال پیام با موفقیت انجام شد';
                break;
            case 'SendNumberGroupAction':
                $numbers= explode(';', $_POST['SendNumberGroupNumberGroupID']);
                
                $Result_SendNumberGroup = $SMS->SendNumberGroup($numbers, $_POST['SendNumberGroupMessageBody'], $_POST['SendNumberGroupSpecialNumber'], isset($_POST['SendNumberGroupIsFlash']), isset($_POST['SendNumberGroupDontSendRepeated']));
                $Status = 'ارسال پیام با موفقیت انجام شد';
            ...
            case 'SendNumberGroupScheduleAction':
                ...
                $Result_SendNumberGroupSchedule = $SMS->SendNumberGroupSchedule($numbers, $_POST['SendNumberGroupScheduleMessageBody'], $_POST['SendNumberGroupScheduleSpecialNumber'], isset($_POST['SendNumberGroupScheduleIsFlash']), isset($_POST['SendNumberGroupScheduleDontSendRepeated']), $Year, $Month, $Day, $Hour, $Minute);
                $Status = 'ارسال پیام زمانبندی با موفقیت انجام شد';
            ...
            case 'InsertNumberInNumberGroup':
                ...
                $R = $SMS->InsertNumberInNumberGroup($_POST['InsertNumberInNumberGroupNumberGroupID'], $numbers, $names);
				foreach($R as $i){
                    $Result_InsertNumberInNumberGroup = $Result_InsertNumberInNumberGroup . $i . '<br />';
                }
                $Status = 'شماره های مورد نظر ثبت شد';
                break;
                
            case 'GetInboxMessageAction':
                $Result_GetInboxMessage = $SMS->GetInboxMessage($_POST['GetInboxMessageNumberOfMessage']);
			break;
            case 'GetInboxMessageWithNumberAction':
                $Result_GetInboxMessageWithNumber = $SMS->GetInboxMessageWithNumber($_POST['GetInboxMessageWithNumberNumberOfMessage'], $_POST['GetInboxMessageWithNumberSpecialNumber']);
                break;
            case 'GetCreditAction':
                $Result_GetCreditAction = $SMS->GetCredit();
			...
			if($_POST['GetInboxMessageWithLastInboxIDReaded'] == 'true') 
                ...
                $Result_GetInboxMessageWithLastInboxID = $SMS->GetInboxMessageWithInboxID($_POST['GetInboxMessageWithLastInboxIDCount'], $_POST['GetInboxMessageWithLastInboxIDID'],$IsReaded);
			break;	
				
			case 'GetUserInfoAction':
                $Result_GetUserInfoAction = $SMS->GetUserInfo();
			break;	
        }
    }
...
<form method="POST">
    ...
    <div class='dForm'>
        ...
        <div class='dLeft'>
            ...
            <div class='m'><textarea name="SendArrayScheduleCheckingMessageID" rows="2" cols="20" id="SendArrayCheckingMessageID" value="<?php echo val("SendArrayCheckingMessageID"); ?>" style="height:50px;"></textarea></div>
